...
def run_max_flow(graph_path, source, sink, mad_flow_script):
    ...
    try:
        result = subprocess.run(
            ['python3', mad_flow_script, '-g', graph_path, '-s', source, '-t', sink, '--json'],
            capture_output=True,
            text=True,
            check=True,
...
def benchmark_graphs(input_dir, output_dir, algorithm, graph_types, num_runs, source, sink, mad_flow_script):
    """Benchmark all graphs in the input directory."""
    ...
    # Process each graph type directory
    for subdir in sorted(input_path.iterdir()):
        ...
        print(f"\nProcessing {normalized_type} graphs from {subdir.name}/")
        ...
        for graph_file in graph_files:
            ...
            print(f"  Benchmarking {graph_file.name}...")
            ...
            for run in range(num_runs):
                ...
                if error:
                    ...
                    print(f"    Run {run+1}/{num_runs}: ERROR - {error}")
                elif elapsed is not None:
                    times.append(elapsed)
                    max_flow_value = flow
                    print(f"    Run {run+1}/{num_runs}: {elapsed:.6f}s")

            if not times:
                print(f"    WARNING: All runs failed for {graph_file.name}")
                continue
            ...
            print(f"    Stats: min={stats['min']:.6f}s, mean={stats['mean']:.6f}s, "
                  f"max={stats['max']:.6f}s, median={stats['median']:.6f}s, "
                  f"stddev={stats['stddev']:.6f}s")

        # Save results for this graph type
        if results:
            save_results(output_path, algorithm, normalized_type, results)
            print(f"  Saved results for {normalized_type}")
...
def save_results(output_path, algorithm, graph_type, results):
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'graph_file', 'num_vertices', 'num_edges', 'max_flow',
            'successful_runs', 'failed_runs',
            'min_time', 'max_time', 'mean_time', 'median_time', 'stddev_time'
        ])
